...
// [F0906-24] Its unit tests (this file's _test.go: six top-level tests
// covering validateFlags' allowlist, runEval's no-API-key early return, and
// writeResults' JSON shape and exit codes) ARE wired into build/Taskfile.yml's
// lint and test targets. They are safe there for a specific reason, and it is
// not "they only check flags": they never execute the built binary, and the
// three that call runEval deliberately leave ANTHROPIC_API_KEY unset or empty,
// so runEval returns at its LookupEnv guard before constructing any provider
// client — no provider call, no network. A future test that supplies a real
// key would break that invariant. What stays deliberately NOT wired in is
// test-integration, and — regardless of which target runs — actually
// executing the built binary bin/wbt-eval, because future categories will
// call out to a real LLM
