...
func TestCheckHandoffNoise(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			ctxSummary: "Wrote noise.go, still need tests",
			wantNoisy:  false,
		},
		{
...
// lineSeparator and paragraphSeparator are built from their hex code points
// (rather than embedded as literal bytes) so the invisible U+2028/U+2029
// characters can't be silently dropped or normalised by text-processing
// layers between this source file and disk.
var (
...
func TestCheckCommandField(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			value:     "before" + lineSeparator + "after",
			wantNoisy: true,
			wantMsg:   "separator",
		},
		{
			name:      "Unicode paragraph separator U+2029 is rejected",
			value:     "before" + paragraphSeparator + "after",
			wantNoisy: true,
			wantMsg:   "separator",
		},
	}
