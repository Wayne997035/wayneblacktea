...
-- outcome.go's CreateOutcome and FinalizeDraft always supply an explicit
-- value on every write (mirrors created_at's own explicit-value-on-every-
-- INSERT pattern in that file, which similarly never leans on a DB-level
-- DEFAULT for correctness), so the column is never actually NULL in
-- practice once this migration has run.
--
