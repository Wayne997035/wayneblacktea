-- SQLite Migration 000026 rollback: recreate ALL foreign-key constraints.
--
-- Mirror of 000026.up.sql; rebuilds each table with the original
-- REFERENCES / ON DELETE clauses restored.

PRAGMA foreign_keys = OFF;

BEGIN TRANSACTION;

-- 1. projects: restore goal_id REFERENCES goals(id)
...
-- 2. tasks: restore project_id REFERENCES projects(id)
...
-- 3. activity_log: restore project_id REFERENCES projects(id)
...
-- 4. session_handoffs: restore project_id REFERENCES projects(id)
...
-- 5. decisions: restore project_id REFERENCES projects(id)
...
-- 6. review_schedule: restore concept_id REFERENCES concepts(id) ON DELETE CASCADE
...
-- 7. work_sessions: restore project_id and current_task_id FKs
...
-- 8. work_session_tasks: restore session_id and task_id FKs
