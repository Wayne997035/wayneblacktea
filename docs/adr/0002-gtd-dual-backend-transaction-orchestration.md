...
- This exception is scoped to dual-backend transactional methods in `internal/gtd`/`internal/storage/sqlite` specifically. It does not license a general Service/Repository layer elsewhere in the codebase — the no-Service/Repository-split rule stands everywhere a single backend has a single owner.
